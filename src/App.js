...
function App() {
  ...
  if (loading) return (
    <div style={{ minHeight:"100vh", display:"flex", alignItems:"center",
      ...
      <div style={{ fontFamily:"Fraunces,serif", fontSize:28, color:"#9E8579", fontWeight:300 }}>Loading…</div>
    </div>
  );
  ...
  if (user) {
    ...
    if (page === "screening-history") return <ScreeningHistory {...pageProps} />;
    ...
    <div>
      {isRegistering ? (
        <><Register setUser={setUser} />
          <p className="switch-text" onClick={() => setIsRegistering(false)}>Already have an account? Login</p></>
      ) : (
        <><Login setUser={setUser} />
          <p className="switch-text" onClick={() => setIsRegistering(true)}>Create Account</p></>
      )}
      <p className="switch-text" style={{ marginTop:8, opacity:0.6 }} onClick={() => setShowAuth(false)}>← Back to home</p>
    </div>
  );
